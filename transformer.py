...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_size, max_len=512, N=1_000.0):
        ...
        div_term = torch.exp(torch.arange(0, embed_size, 2) * -(np.log(N) / embed_size))
        pe = torch.zeros(max_len, embed_size)
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        # TODO create the positional encoding by filling in the values for pe
        self.pe = pe.unsqueeze(0).transpose(0, 1)  # reshape to (max_len, 1, embed_size) so that it applies to each sequence in the mini-batch
...
class TransformerLayer(nn.Module):
    def __init__(self, embed_size, nhead, layer_width=256, activation=F.relu):
        ...
        self.self_attn = nn.MultiheadAttention(embed_size, nhead, batch_first=True)
    ...
    def forward(self, x, attn_mask, need_attn_weights=False):
        ...
        # Normalize input
        xnorm = self.norm1(x)  # Use self.norm1 instead of self.ln1

        # Self-attention
        att, att_weights = self.self_attn(
            xnorm, xnorm, xnorm, attn_mask=attn_mask,
            need_weights=need_attn_weights, average_attn_weights=False
        )

        # Add attention output to original input
        y = x + self.dropout(att)

        # Layer norm and feedforward
        ynorm = self.norm2(y)
        z = self.feedforward(ynorm)

        # Add feedforward output to original residual
        z = y + self.dropout(z)
...
class Transformer(nn.Module):
    def __init__(self, vocab_size, embed_size=32, num_layers=2, nhead=8, layer_width=256, activation=F.relu, max_len=1024+32, N=1_000.0, pos_enc=True):
        ...
        # Layer norm and final linear layer
